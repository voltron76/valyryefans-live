  ...
  return `
    <div class="post-card animate-fade-in-up" data-post-id="${item.id}" data-type="${item.type || 'photo'}">
      ...
      <div class="post-card__actions">
        <button class="post-action ${item.likedByUser ? 'post-action--active' : ''}" data-action="like" data-id="${item.id}" aria-label="${item.likedByUser ? 'Unlike' : 'Like'} (${item.likes || 0})">
          <span aria-hidden="true">${item.likedByUser ? icons.heartFilled : icons.heart} <span class="like-count">${item.likes || 0}</span></span>
        </button>
        <button class="post-action" data-action="comment" data-id="${item.id}" aria-label="Comment (${commentsArr.length || 0})">
          <span aria-hidden="true">${icons.comment} <span>${commentsArr.length || 0}</span></span>
        </button>
        <button class="post-action" data-action="tip" data-id="${item.id}" aria-label="Tip">
      ...
      function openStoryViewer(startCircleIndex) {
        ...
        likeBtn?.addEventListener('click', async (e) => {
          ...
          if (feedCard) {
            ...
            feedCard.setAttribute('aria-label', `${liked ? 'Unlike' : 'Like'} (${likesCount})`);
          }
        });
      ...
      storeUnsubscribe = subscribe(['content', 'polls'], (newState) => {
        ...
        newState.content?.forEach(item => {
          ...
          if (likeBtn) {
            ...
            likeBtn.setAttribute('aria-label', `${liked ? 'Unlike' : 'Like'} (${item.likes || 0})`);
          }
        });
